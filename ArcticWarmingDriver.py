...
This driver script imports functions from ArcticWarmingUtilities.py
and receives the netCDF file to be processed as the command line argument.
It generates two netCDF files as the output and saves them in the /data directory: 
1) ds with warming trend computed for each grid cell globally where the cells that are statistically non-sign are masked out
2) ds with amplification computed for each grid cell globally

@author: aslibese
"""

# import the functions from ArcticWarmingUtilities.py
from ArcticWarmingUtilities import selectPeriod, find_variable_by_dims, annualMean, performLinearRegression, gridCellRegression, weightedAverage
...
trend_output_file_path = os.path.join(input_directory, "{}_trendMasked.nc".format(filename_without_ext))
...
# construct the output file path
aa_output_file_path = os.path.join(input_directory, "{}_aa.nc".format(filename_without_ext))

# save the processed dataset
ds_processed.to_netcdf(aa_output_file_path)

print("Processed amplification data saved to:", aa_output_file_path)
